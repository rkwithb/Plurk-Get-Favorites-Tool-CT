...
/api/plurks query parameters:
    ...
    plurk_type=int filter by type (0=public, 1=private,
    sort=str sort order: "newest" (default only in v1)

/api/tags query parameters:
    month=YYYY_MM filter tags to specific month

Tag request body (JSON):
    {"plurk_id": 123, "tag_name": "my tag"}

core/ hard rule: server.py imports from core/db.py only,
...
from core.paths import BACKUP_DIR, BASE_DIR
...
# Active port — set by start_server().
# Used by stop_server() and wait_until_ready()
_port: int = 5123
...
def _create_app() -> Flask:
    ...
    def api_plurks():
        ...
        Query params:
            month=YYYY_MM filter by month
            nick_name=str filter by owner nick name
            plurk_type=int filter by type code
            sort=str sort order key ("newest" in v1)
        """
        ...
        try:
            ...
            logger.error(
                "server: /api/plurks query failed — %s", e
            )
    ...
    def api_tags_list():
        """
        List tags in database, optionally filtered by month.
    ...
    def api_tags_add():
        """
        Add a tag to a plurk. Create if not exists.
        ...
        if not plurk_id or not tag_name:
            return jsonify(
                {
                    "error": "plurk_id and tag_name are required"
                }
            ), 400
        ...
        try:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO plurk_tags "
                "(plurk_id, tag_id) VALUES (?, ?)",
                (plurk_id, tag_id),
            )
            _conn.commit()
            logger.debug(
                "server: tag '%s' added to plurk_id=%s",
                tag_name,
                plurk_id,
            )
    ...
    def api_tags_remove():
        ...
        try:
            ...
            tag_id = row[0]
            cursor.execute(
                "DELETE FROM plurk_tags WHERE plurk_id = ? "
                "AND tag_id = ?",
                (plurk_id, tag_id),
            )
            _conn.commit()
            logger.debug(
                "server: tag '%s' removed from plurk_id=%s",
                tag_name,
                plurk_id,
            )
...
def _build_plurk_list(rows) -> list:
    """
    Build trimmed plurk list from DB rows for API responses.
    Mirrors JS object shape from export.py so index.html
    ...
    for row in rows:
        try:
            ...
            url = f"https://www.plurk.com/p/{base36_encode(plurk_id)}"
            obj["plurk_url"] = url
            ...
            logger.warning(
                "server: skipping malformed row plurk_id=%s — %s",
                row["plurk_id"],
                e,
            )
...
def start_server(conn: sqlite3.Connection, port: int) -> None:
    ...
    Safe to call only once per process — subsequent calls
    ignored if server already running.

    Args:
        conn: open database connection shared with GUI
...
def stop_server() -> None:
    """
    Signal Flask server to shut down.
    Built-in shutdown only in request context, so use
    werkzeug shutdown via one-shot internal request.
...
def wait_until_ready(port: int, timeout: float = 10.0) -> bool:
    """
    Poll GET /health until server responds with 200 or timeout.
    Called by GUI between start_server() and webbrowser.open().
    ...
    Returns:
        True if server ready within timeout, False otherwise.
    """
