...
2. 前往 [Plurk API 申請頁面](https://www.plurk.com/API) 取得你的：
   - **Consumer Key** — 填入對應欄位
   - **Consumer Secret** — 填入對應欄位
3. 將這兩項資訊填入「**Consumer Key**」和「**Consumer Secret**」欄位中。
4. 點選「**儲存金鑰**」按鈕。
5. 點選「**授權登入**」，瀏覽器會自動開啟噗浪授權頁面。
6. 使用你的噗浪帳號登入（如果還沒登入的話），然後授權本工具存取你的最愛資料。
7. 授權完成後，複製頁面上的驗證碼，回到應用程式輸入驗證碼並點選「**確認**」。
8. 應用程式會自動填入 **Access Token** 和 **Token Secret**，按下「**儲存金鑰**」完成設定。
...
這通常是因為授權令牌（Access Token）已過期（通常在一段時間後）。直接再按工具畫面的「**授權登入**」按鈕，完成一次新的 OAuth 授權即可。
